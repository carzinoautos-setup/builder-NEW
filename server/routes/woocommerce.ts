...
const validateWooCommerceConfig = () => {
  ...
  if (missing.length > 0) {
    console.error('❌ Missing WooCommerce environment variables:', missing.join(', '));
    return false;
  }
  return true;
};
...
function transformWooCommerceToVehicle(product: any): any {
  ...
  return {
    ...
    transmission_speed: getMetaValue('vehicle_transmission_speed') || 'Automatic',
  ...
  try {
    if (!wcApi) {
      return res.status(500).json({
        success: false,
        message: 'WooCommerce API not configured. Please set environment variables.',
        data: [],
        meta: {
          totalRecords: 0,
          totalPages: 0,
    ...
    console.log(`✅ Returned ${vehicles.length} vehicles from WooCommerce (page ${page}/${totalPages})`);
    ...
    res.status(500).json({
      success: false,
      message: error instanceof Error ? error.message : 'Failed to fetch vehicles from WooCommerce',
      data: [],
      meta: {
        totalRecords: 0,
        totalPages: 0,
