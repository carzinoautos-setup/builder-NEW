...
async function runApiTests() {
  ...
  const tests: Array<{ endpoint: string; description: string }> = [
    ...
    { endpoint: '/vehicles?page=1&pageSize=10', description: 'Vehicles pagination (page 1)' },
    { endpoint: '/vehicles?page=2&pageSize=20', description: 'Vehicles pagination (page 2)' },
    { endpoint: '/vehicles?make=Toyota', description: 'Vehicles filtered by make' },
    { endpoint: '/vehicles?condition=New&maxPrice=50000', description: 'Vehicles with multiple filters' },
  ...
  if (successful > 0) {
    ...
    for (const perfTest of performanceTests) {
      ...
      if (result.success && result.data) {
        console.log(`   📈 ${perfTest.description}: ${result.responseTime}ms (${result.data.meta?.totalRecords || 0} total records)`);
      }
    }
  }
