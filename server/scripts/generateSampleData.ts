...
export async function generateSampleData(recordCount: number = 50000): Promise<void> {
  console.log(`🚀 Starting to generate ${recordCount} sample vehicle records...`);
  ...
  try {
    ...
    const [countResult] = await db.execute('SELECT COUNT(*) as count FROM vehicles');
    const existingCount = (countResult as any)[0].count;
    
    if (existingCount > 0) {
      console.log(`ℹ️  Found ${existingCount} existing records. Skipping data generation.`);
      return;
    }
    ...
    for (let batch = 0; batch < batches; batch++) {
      ...
      console.log(`📊 Progress: ${progress}/${recordCount} records inserted (${((progress/recordCount)*100).toFixed(1)}%)`);
    }
    
    console.log('✅ Sample data generation completed successfully!');
    
    // Display summary
    const [finalCount] = await db.execute('SELECT COUNT(*) as count FROM vehicles');
