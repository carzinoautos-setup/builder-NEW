...
const MAKES = ['Audi', 'BMW', 'Chevrolet', 'Ford', 'Honda', 'Hyundai', 'Mercedes-Benz', 'Nissan'];
const MODELS_BY_MAKE = {
  'Audi': ['A3', 'A4', 'A6', 'Q5', 'Q7', 'Q8'],
  'BMW': ['3 Series', '5 Series', 'X3', 'X5', 'X7'],
  'Chevrolet': ['Silverado', 'Equinox', 'Malibu', 'Traverse', 'Camaro', 'Tahoe'],
  'Ford': ['F-150', 'Escape', 'Explorer', 'Mustang', 'Edge', 'Expedition', 'Ranger'],
...
const TRIMS = ['Base', 'LX', 'EX', 'EX-L', 'Touring', 'Sport', 'Limited', 'Premium', 'Luxury', 'SE', 'SL', 'SR', 'Platinum', 'Lariat', 'XLT'];
...
const DEALERS = [
  'Bayside Ford', 'Premium Auto Group', 'Downtown Honda', 'City Toyota', 
  'Luxury Motors', 'Northwest Chevrolet', 'Eastside BMW', 'Metro Audi',
  'Pacific Mercedes-Benz', 'Summit Hyundai', 'Valley Nissan'
];
...
function generateSimpleVehicleRecord(id: number): SimpleVehicleRecord {
  ...
  const yearFactor = condition === 'New' ? 1 : Math.max(0.4, 1 - (2024 - year) * 0.08);
...
export class SimpleMockVehicleService {
  ...
  constructor() {
    console.log('🔧 SimpleMockVehicleService: Generating sample data (original demo format)...');
    this.generateMockData();
    console.log(`✅ SimpleMockVehicleService: ${this.vehicles.length} vehicles generated`);
  }
  ...
  private matchesFilters(vehicle: SimpleVehicleRecord, filters: SimpleVehicleFilters): boolean {
    // Condition filter
  ...
  async getFilterOptions(): Promise<{
    ...
    const makes = Array.from(new Set(
      this.vehicles.map(v => v.title.split(' ')[1])
    )).sort();
