...
import { VehicleWithLocation, LocationFilters, SellerRecord } from "../types/seller";
...
export class LocationService {
  ...
  async getVehiclesWithinRadius(
    ...
    let whereConditions = [`seller_latitude BETWEEN ? AND ?`, `seller_longitude BETWEEN ? AND ?`];
    let params = [minLat, maxLat, minLng, maxLng];

    // Add other filters
    if (filters.make) {
      whereConditions.push(`make IN (${filters.make.split(',').map(() => '?').join(',')})`);
      params.push(...filters.make.split(','));
    }
    
    if (filters.condition) {
      whereConditions.push(`condition IN (${filters.condition.split(',').map(() => '?').join(',')})`);
   ...
   */
  async syncSellerCoordsToVehicles(sellerAccountNumber?: string): Promise<void> {
    ...
    try {
      ...
      console.log(`✅ Synced seller coordinates to vehicles${sellerAccountNumber ? ` for seller ${sellerAccountNumber}` : ''}`);
    } catch (error) {
  ...
  async createOptimalIndexes(): Promise<void> {
    ...
    for (const indexSQL of indexes) {
      try {
        ...
        console.log('✅ Created index/table:', indexSQL.substring(0, 50) + '...');
      } catch (error) {
