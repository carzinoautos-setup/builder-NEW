...
import { calculateMonthlyPayment, calculateAffordablePrice, PaymentParams, PaymentResult } from '../lib/paymentCalculator';
...
export function usePaymentFilters({
  ...
  const [affordablePriceRange, setAffordablePriceRange] = useState<{min: number, max: number} | null>(null);
  ...
  }, [paymentState.paymentMin, paymentState.paymentMax, paymentState.interestRate, paymentState.loanTermMonths, paymentState.downPayment]);
  ...
  const calculateVehiclePayment = useCallback((salePrice: number): PaymentResult | null => {
    try {
  ...
  const calculateBulkPayments = useCallback(async (vehicles: VehicleWithPayment[]): Promise<VehicleWithPayment[]> => {
    try {
      setIsCalculating(true);
      ...
      const response = await fetch('/api/payments/bulk', {
        ...
        body: JSON.stringify({
          vehicles: vehicles.map(v => ({ id: v.id, salePrice: v.salePrice })),
          downPayment: parseFloat(paymentState.downPayment) || 0,
          interestRate: parseFloat(paymentState.interestRate) || 0,
          loanTermMonths: parseInt(paymentState.loanTermMonths) || 60,
        }),
      ...
      return vehicles.map(vehicle => {
        const calculation = result.data.find((calc: any) => calc.vehicleId === vehicle.id);
        return {
          ...vehicle,
          calculatedPayment: calculation ? Math.round(calculation.monthlyPayment) : undefined,
  ...
  const updatePaymentState = useCallback((updates: Partial<PaymentFilterState>) => {
  ...
  const vehicleMatchesPaymentFilter = useCallback((salePrice: number): boolean => {
    if (!affordablePriceRange) return true;
    
    return salePrice >= affordablePriceRange.min && salePrice <= affordablePriceRange.max;
