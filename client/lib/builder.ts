...
if (!BUILDER_API_KEY) {
  console.warn(
    "⚠️ VITE_BUILDER_API_KEY not found. Builder.io integration will not work.",
  );
} else {
...
export async function getBuilderContent(
  ...
  try {
    ...
    console.error(
      `Failed to fetch Builder.io content for model "${modelName}":`,
      error,
    );
    return null;
  }
...
export function transformVehiclesForBuilder(vehicles: any[]) {
  return vehicles.map((vehicle) => ({
    ...
    price: vehicle.price,
    formattedPrice:
      vehicle.formattedPrice || `$${vehicle.price?.toLocaleString()}`,
    ...
    formattedMileage:
      vehicle.formattedMileage || `${vehicle.mileage?.toLocaleString()} mi`,
